...
bool EmployeeService::Find(int find_) {
  ...
  bool dni_find = false;
  ifstream registers("db/empleados.csv", ios::in);
  if (!registers) {
...
void EmployeeService::Add(EmployeeModel empleado) {
  ...
  if (Find(empleado.get_dni())) {
    ...
    ofstream doc_employeed("db/empleados.csv", ios::app);
    if (!doc_employeed) {
      cout << "archivo no creado" << endl;
    } else {

      doc_employeed << empleado.ToString() << endl; // corregir
    }
  }
}
...
void EmployeeService::Remove(int id_remove) {
  ...
  ifstream doc_employeed("db/empleados.csv", ios::in);
  ...
  ofstream doc_delate("db/empleados.csv");
...
void EmployeeService::Read() {
  ...
  while (getline(doc_employeed, data)) {
    stringstream registers(
        data); // convertimos la cadena de datos en un flujo de datos
...
void EmployeeService::Update(int dni_) {
  ...
  while (getline(doc_employeed, data)) {
    ...
    if (dni_ == stoi(dni)) {
      ...
      cin >> dni;
      update_employeed.push_back(id + "," + name + "," + age + "," + wage +
                                 "," + timetable + "," + dni);
    } else {
  ...
  if (find_employeed) {
    ...
    ofstream doc_update("db/empleados.csv"); // creamos un nuevo documento
    for (int i = 0; i < update_employeed.size(); i++) { // leemos el vector
      doc_update << update_employeed[i]
                 << endl; // guardamos los nuevos datos en el documeto
    }
    doc_update.close();
